...
def generate_xml(filename, random_locations, labels):
    #######  创建XML文件  ########
    list_top = []
    list_bndbox = []
    ...
    for i, loc in enumerate(random_locations):
        xmin, ymin, xmax, ymax, down = loc

        list_bndbox.extend([labels[i], flag, pose, truncated, difficult,
...
def generate_random_location(number, x1, y1, x2, y2, w, h):
    '''
    ...
    while len(locations) < number and attempts < max_attempts:
        down = random.choice([0, 1])
        if down == 0:
            xmin = random.randint(x1, x1 + w - pin_width)
            ymin = y1
        else:
            xmin = random.randint(x2, x2 + w - pin_width)
            ymin = y2

        xmax = xmin + pin_width
        ymax = ymin + pin_height

        new_location = (xmin, ymin, xmax, ymax, down)
...
def generate_image(image, random_locations):
    ...
    generate_xml(image_filename, random_locations, labels)


def generate_image_draw(image, random_locations):
    '''
    从针脚图像的文件夹中随机读取针脚图像，将图像按照locations中的坐标融合于模版图像image，数量取决于locations长度，
    如果down为0，需要将针脚图像旋转180度
    ...
    for loc in random_locations:
        ...
        # 绘制标注框
        label = pin_image_path[-7:-5]  # 获取文件名最后两个字母作为标签
        if label == 'OK':
            color = (0, 255, 0)  # 绿色
        elif label == 'NG':
            color = (0, 0, 255)  # 红色
        else:
            color = (255, 255, 255)  # 白色，默认颜色
    ...
    output_folder = "gen_data/generate_data_draw"  # 存放生成图像的文件夹
    os.makedirs(output_folder, exist_ok=True)
